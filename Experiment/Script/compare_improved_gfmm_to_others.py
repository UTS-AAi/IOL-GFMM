...
tetaRange = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8]
...
def running_online_gfmm(xTr, patClassIdTr, xVal, patClassIdVal, teta = 0.26):
    ...
    result = olnClassifier.predict(xVal, xVal, patClassIdVal, False)
    err = 1
    if result != None:
...
def running_online_gfmm_manhattan(xTr, patClassIdTr, xVal, patClassIdVal, teta = 0.26):
    olnClassifier = OnlineGFMM(gamma = 1, teta = teta, tMin = teta, isDraw = False, oper = 'min', isNorm = False)
    olnClassifier.fit(xTr, xTr, patClassIdTr)
            
    result = olnClassifier.predict(xVal, xVal, patClassIdVal, True)
    err = 1
    if result != None:
        err = result.summis / len(patClassIdVal)
...
def running_improved_online_gfmm_prob(xTr, patClassIdTr, xVal, patClassIdVal, teta = 0.26):
    impolnClassifier = ImprovedOnlineGFMM(gamma = 1, teta = teta, sigma = 1 - teta, isDraw = False, oper = 'min', isNorm = False)
    impolnClassifier.fit(xTr, xTr, patClassIdTr)
            
    result = impolnClassifier.predict(xVal, xVal, patClassIdVal, True)
    err = 1
    if result != None:
        err = result.summis / len(patClassIdVal)
...
def find_optimal_param_decision_tree(fold1Data, fold1Label, fold2Data, fold2Label, fold3Data, fold3Label, maxDepthRange = [3, 30]):
    ...
    for maxDepth in maxDepthRange:      
        ...
        # using fold2 as validation, fold1 and fold3 are training sets
        trainingData = np.vstack((fold3Data, fold1Data))
        trainingLabel = np.append(fold3Label, fold1Label)
...
def find_optimal_param_online_gfmm_manhattan(fold1Data, fold1Label, fold2Data, fold2Label, fold3Data, fold3Label):
    minErr = 2
    tetaOpt = 0.26
    ...
    for teta in tetaRange:
        ...
        error = error + running_online_gfmm_manhattan(trainingData, trainingLabel, fold1Data, fold1Label, teta)
    
        # using fold2 as validation, fold1 and fold3 are training sets
        trainingData = np.vstack((fold3Data, fold1Data))
        trainingLabel = np.append(fold3Label, fold1Label)
        
        error = error + running_online_gfmm_manhattan(trainingData, trainingLabel, fold2Data, fold2Label, teta)
        
        # using fold3 as validation, fold1 and fold2 are training sets
        trainingData = np.vstack((fold1Data, fold2Data))
        trainingLabel = np.append(fold1Label, fold2Label)
        
        error = error + running_online_gfmm_manhattan(trainingData, trainingLabel, fold3Data, fold3Label, teta)
...
def find_optimal_param_improved_online_gfmm_prob(fold1Data, fold1Label, fold2Data, fold2Label, fold3Data, fold3Label):
    minErr = 2
    tetaOpt = 0.26
    ...
    for teta in tetaRange:
        ...
        error = error + running_improved_online_gfmm_prob(trainingData, trainingLabel, fold1Data, fold1Label, teta)
    
        # using fold2 as validation, fold1 and fold3 are training sets
        trainingData = np.vstack((fold3Data, fold1Data))
        trainingLabel = np.append(fold3Label, fold1Label)
        
        error = error + running_improved_online_gfmm_prob(trainingData, trainingLabel, fold2Data, fold2Label, teta)
        
        # using fold3 as validation, fold1 and fold2 are training sets
        trainingData = np.vstack((fold1Data, fold2Data))
        trainingLabel = np.append(fold1Label, fold2Label)
        
        error = error + running_improved_online_gfmm_prob(trainingData, trainingLabel, fold3Data, fold3Label, teta)
...
def find_optimal_param_agglo_2(fold1Data, fold1Label, fold2Data, fold2Label, fold3Data, fold3Label):
    ...
    similRange = [0.9, 0.8, 0.5, 0.2, 0.1]
    
    for teta in tetaRange:
        for simil in similRange:
            error = 0
            # using fold1 as validation, fold2 and fold3 are training sets
            trainingData = np.vstack((fold2Data, fold3Data))
            trainingLabel = np.append(fold2Label, fold3Label)
        
            error = error + running_agglo_2(trainingData, trainingLabel, fold1Data, fold1Label, teta, simil)
        
            # using fold2 as validation, fold1 and fold3 are training sets
            trainingData = np.vstack((fold3Data, fold1Data))
            trainingLabel = np.append(fold3Label, fold1Label)
            
            error = error + running_agglo_2(trainingData, trainingLabel, fold2Data, fold2Label, teta, simil)
            
            # using fold3 as validation, fold1 and fold2 are training sets
            trainingData = np.vstack((fold1Data, fold2Data))
            trainingLabel = np.append(fold1Label, fold2Label)
            
            error = error + running_agglo_2(trainingData, trainingLabel, fold3Data, fold3Label, teta, simil)
...
if __name__ == '__main__':
    ...
    save_imponline_gfmm_prob_result_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/improved_online_gfmm_probability/'
    save_online_gfmm_manhattan_result_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/online_gfmm_manhattan/'
    save_online_gfmm_result_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/Online_GFMM/'
    save_efmnn_result_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/EFMNN/'
    save_knefmnn_result_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/KNEFMNN/'
    save_fmnn_result_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/FMNN/'
    
    save_result_KNN_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/KNN/'
    save_result_SVM_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/SVM/'
    save_result_DT_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/Decision_Tree/'
    save_result_NB_folder_path = root_path + '/Experiment/modified_online_gfmm/compare/Naive_Bayes/'
    ...
    for dt in range(len(dataset_names)):
        ...
        optimization_value_online_gfmm_manhattan_save = np.array([], dtype=np.str)
        optimization_time_online_gfmm_manhattan_save = np.array([])
        ...
        optimization_value_imponline_gfmm_prob_save = np.array([], dtype=np.str)
        optimization_time_imponline_gfmm_prob_save = np.array([])
        ...
        for fo in range(4):
            if fo == 0:
                ...
                numTestSample = len(testingLabel)
                f1dt, f2dt, f3dt = fold3Data, fold4Data, fold1Data
                f1lb, f2lb, f3lb = fold3Label, fold4Label, fold1Label
            elif fo == 2:
                ...
                numTestSample = len(testingLabel)
                f1dt, f2dt, f3dt = fold4Data, fold1Data, fold2Data
                f1lb, f2lb, f3lb = fold4Label, fold1Label, fold2Label
            else:
            ...
            result = olnClassifier.predict(testingData, testingData, testingLabel, False)
            if result != None:
            ...
            # online GFMM with manhattan distance
            time_start = time.perf_counter()
            tetaOnlnGFMMManhattan = find_optimal_param_online_gfmm_manhattan(f1dt, f1lb, f2dt, f2lb, f3dt, f3lb)
            time_end = time.perf_counter()
            optimization_time_online_gfmm_manhattan_save = np.append(optimization_time_online_gfmm_manhattan_save, time_end - time_start)
            optimization_value_online_gfmm_manhattan_save = np.append(optimization_value_online_gfmm_manhattan_save, 'teta = ' + str(tetaOnlnGFMMManhattan))
            
            olnManClassifier = OnlineGFMM(gamma = 1, teta = tetaOnlnGFMMManhattan, tMin = tetaOnlnGFMMManhattan, isDraw = False, oper = 'min', isNorm = False)
            olnManClassifier.fit(trainingData, trainingData, trainingLabel)
            
            training_time_online_gfmm_manhattan_save = np.append(training_time_online_gfmm_manhattan_save, olnManClassifier.elapsed_training_time)
            numhyperbox_online_gfmm_manhattan_save = np.append(numhyperbox_online_gfmm_manhattan_save, len(olnManClassifier.classId))            
                
            result = olnManClassifier.predict(testingData, testingData, testingLabel, True)
            if result != None:
                err = np.round(result.summis / numTestSample * 100, 3)
                testing_error_online_gfmm_manhattan_save = np.append(testing_error_online_gfmm_manhattan_save, err)
            
            # improved online gfmm with probability
            time_start = time.perf_counter()
            tetaImpOnline = find_optimal_param_improved_online_gfmm_prob(f1dt, f1lb, f2dt, f2lb, f3dt, f3lb)
            time_end = time.perf_counter()
            optimization_time_imponline_gfmm_prob_save = np.append(optimization_time_imponline_gfmm_prob_save, time_end - time_start)
            optimization_value_imponline_gfmm_prob_save = np.append(optimization_value_imponline_gfmm_prob_save, 'teta = ' + str(tetaImpOnline))
            
            impolnClassifier = ImprovedOnlineGFMM(gamma = 1, teta = tetaImpOnline, sigma = 1 - tetaImpOnline, isDraw = False, oper = 'min', isNorm = False)
    
            impolnClassifier.fit(trainingData, trainingData, trainingLabel)
            
            training_time_imponline_gfmm_prob_save = np.append(training_time_imponline_gfmm_prob_save, impolnClassifier.elapsed_training_time)
            numhyperbox_imponline_gfmm_prob_save = np.append(numhyperbox_imponline_gfmm_prob_save, len(impolnClassifier.classId))            
                
            result = impolnClassifier.predict(testingData, testingData, testingLabel, True)
            if result != None:
                err = np.round(result.summis / numTestSample * 100, 3)
        ...
        # save online gfmm with manhattan results
        data_online_gfmm_manhattan_save = np.hstack((fold_index.reshape(-1, 1), numhyperbox_online_gfmm_manhattan_save.reshape(-1, 1), training_time_online_gfmm_manhattan_save.reshape(-1, 1), testing_error_online_gfmm_manhattan_save.reshape(-1, 1), optimization_time_online_gfmm_manhattan_save.reshape(-1, 1), optimization_value_online_gfmm_manhattan_save.reshape(-1, 1)))
        ...
        with open(filename_online,'a') as f_handle:
            f_handle.writelines('Fold, No hyperboxes, Training time, Testing error, Optimization time, Optimal param\n')
            np.savetxt(f_handle, data_online_gfmm_manhattan_save, fmt='%s', delimiter=', ')
        
        # save improved gfmm prob
        data_imponline_gfmm_prob_save = np.hstack((fold_index.reshape(-1, 1), numhyperbox_imponline_gfmm_prob_save.reshape(-1, 1), training_time_imponline_gfmm_prob_save.reshape(-1, 1), testing_error_imponline_gfmm_prob_save.reshape(-1, 1), optimization_time_imponline_gfmm_prob_save.reshape(-1, 1), optimization_value_imponline_gfmm_prob_save.reshape(-1, 1)))
        filename_online = save_imponline_gfmm_prob_result_folder_path + dataset_names[dt] + '.csv'
        
        open(filename_online, 'w').close() # make existing file empty
        
        with open(filename_online,'a') as f_handle:
            f_handle.writelines('Fold, No hyperboxes, Training time, Testing error, Optimization time, Optimal param\n')
            np.savetxt(f_handle, data_imponline_gfmm_prob_save, fmt='%s', delimiter=', ')
