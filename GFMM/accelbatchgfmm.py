        ...
        while isTraining:
            ...
            while k < len(self.classId):
                ...
                if len(maxB) > 0:
                    ...
                    idx_same_classes = (self.classId[indmaxB] == self.classId[k]) | (self.classId[indmaxB] == UNLABELED_CLASS) | ((self.classId[indmaxB] != self.classId[k]) & (self.classId[k] == UNLABELED_CLASS))
...
#                        newW[tmp_row] = np.maximum(self.W[row1], self.W[row2])
                        ...
                        if ((((newW[int(pairewise_maxb[i, 0])] - newV[int(pairewise_maxb[i, 0])]) <= self.teta).all() == True) and (not modifiedIsOverlap(newV, newW, int(pairewise_maxb[i, 0]), newClassId))):
                            self.V = newV
                            self.W = newW
